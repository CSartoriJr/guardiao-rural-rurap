...
import { Loader2, UserPlus, ArrowLeft, Eye, EyeOff, ShieldAlert } from 'lucide-react';
...
export default function CreateAdminForm() {
  ...
  const { user: adminUser, createAdminWithAuth } = useAuth();
  const router = useRouter();

  const { control, handleSubmit, reset, formState: { errors } } = useForm<AdminFormValues>({
  ...
  const onSubmit: SubmitHandler<AdminFormValues> = async (data) => {
    if (!adminUser || !createAdminWithAuth) {
      toast({ title: "Erro", description: "Funcionalidade de criação não está disponível.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const newAdmin = await createAdminWithAuth(adminData);
      
      if (newAdmin) {
        toast({
          title: 'Administrador Criado!',
          description: `O administrador ${newAdmin.name} foi criado com sucesso.`,
        });
        reset();
      } else {
        toast({ title: "Falha na Criação", description: "Não foi possível criar o administrador.", variant: "destructive" });
      }
    } catch (error: any) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
        ...
        <CardHeader>
            <CardTitle className="font-headline text-2xl">Criar Novo Administrador</CardTitle>
            <CardDescription>Preencha os detalhes abaixo para adicionar um novo administrador.</CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="space-y-2">
                ...
                name="name"
                control={control}
                render={({ field }) => <Input id="name" placeholder="Nome do novo administrador" {...field} />}
                />
