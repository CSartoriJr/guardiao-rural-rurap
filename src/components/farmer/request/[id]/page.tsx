...
const StatusDisplay = ({ status, recommendation, technicianName, responseDate, laudoPdfUrl }: Pick<AgriRequest, 'status' | 'recommendation' | 'technicianName' | 'responseDate' | 'laudoPdfUrl'>) => {
  ...
  return (
    <Card className="mt-6 bg-background/50">
      ...
      {status === 'Pending' && (
        <CardContent>
          <p className="text-muted-foreground">Seu Solicitação está atualmente em revisão por um técnico. Você será notificado assim que uma resposta estiver disponível.</p>
        </CardContent>
      )}
    </Card>
  );
    ...
    getRequestById(requestId)
      .then(data => {
        if (data && (data.farmerCpf === user.cpf || user.role === 'admin' || user.role === 'technician')) { 
          ...
          setError("Solicitação não encontrado.");
