...
export default function TechnicianViewRequestPage() {
  ...
  useEffect(() => { // Effect for initial data loading
    ...
    getRequestById(requestId)
        ...
        .catch(err => {
            console.error("[TechnicianViewRequestPage] Falha ao buscar Solicitação do Firestore:", err);
            setError("Falha ao carregar detalhes da Solicitação.");
            toast({title: "Erro ao Carregar", description: "Falha ao carregar detalhes da Solicitação do Firestore.", variant: "destructive"});
        })
        .finally(() => {
            setIsLoading(false);
        });
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {request.status === 'Pending' && user?.role === 'technician' ? (
          ...
          <Card className="mt-6 bg-card/80">
            ...
            <CardContent>
              <p><strong>Status:</strong> {request.status === 'Positive' ? 'Positivo' : request.status === 'Negative' ? 'Negativo' : request.status === 'Inconclusive' ? 'Inconclusivo' : request.status === 'Suspeita de Infecção' ? 'Suspeita de Infecção' : 'Pendente'}</p>
