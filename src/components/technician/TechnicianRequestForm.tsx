...
import { Loader2, Send, LandPlot, AlertTriangle, MapPin, LocateFixed, WifiOff, RefreshCw, XCircle, Users, CalendarIcon, WholeWord, Leaf, Briefcase } from 'lucide-react';
...
export default function TechnicianRequestForm() {
  ...
  const onSubmit: SubmitHandler<RequestFormValues> = async (data) => {
    ...
    if (!selectedFarmer) {
      toast({ title: "Erro de Validação", description: "Agricultor selecionado é inválido. Por favor, recarregue a página.", variant: "destructive" });
      return;
    }
    ...
    <Card className="w-full max-w-2xl mx-auto shadow-lg">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <Label htmlFor="farmerId" className="flex items-center"><Users className="h-4 w-4 mr-2 text-primary" />Agricultor</Label>
            <Controller
              name="farmerId"
              control={control}
              render={({ field }) => (
