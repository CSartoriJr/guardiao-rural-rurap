 ...
 *
 * - generateRecommendation - A function that generates recommendation drafts and extracts/confirms GPS.
...
Your task is to provide a detailed recommendation for the farmer and to determine the GPS coordinates.
...
1.  **GPS Coordinate Determination**:
    *   **Priority**: If 'Device Latitude' and 'Device Longitude' are provided and appear valid (numerical, within range -90 to 90 for lat, -180 to 180 for lon), use these as the primary coordinates. You can visually inspect 'Photo 1 (Panoramic)' to see if it generally matches a rural Amapá setting, but the device coordinates should be preferred if valid.
    *   **Image Extraction (Fallback)**: If device coordinates are *not* provided or are clearly invalid (e.g., non-numerical, out of range), then carefully examine 'Photo 1 (Panoramic)'. Look for any text overlay or embedded information that clearly indicates GPS Latitude and Longitude values (e.g., as added by apps like NoteCam).
    *   If you find clearly visible numerical Latitude and Longitude values on 'Photo 1 (Panoramic)', parse them directly as numbers.
    *   Populate the 'extractedLatitude' and 'extractedLongitude' fields in your JSON output with the determined numerical coordinates (either from device or image extraction). Latitude must be a number between -90 and 90. Longitude must be a number between -180 and 180.
    *   If no valid GPS coordinates can be determined (neither from device nor clearly visible and parsable from images), ensure 'extractedLatitude' and 'extractedLongitude' are omitted or set to null. Do not invent or estimate coordinates if no reliable source is available.

2.  **Agricultural Recommendation**: Based on the plant classification, variety, submitted photos, area information, and the determined GPS coordinates (if any):
    *   Consider the proportion of infected area to planted area if provided.
    *   If you determined GPS coordinates, consider if this location information (e.g. general climate of Amapá, Brazil, or common regional pests/diseases if broadly known for that region based on the coordinates) can refine your diagnosis or recommendation. State if you are using this information.
