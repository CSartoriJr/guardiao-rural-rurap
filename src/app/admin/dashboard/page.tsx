...
import { UserPlus, ListFilter, ClipboardList, Frown } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  return (
    <PageWrapper allowedRoles={['admin']}>
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <div className="flex items-center text-primary bg-primary/10 px-3 py-2 rounded-md text-sm">
            ...
            <span>Total de Pedidos: {requests.length}</span>
          </div>
      ...
      {/* Add filtering options here in the future if needed */}
      {/* <div className="mb-6 flex justify-end">
        <Button variant="outline"><ListFilter className="mr-2 h-4 w-4" /> Filtrar Pedidos</Button>
      </div> */}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Using TechnicianRequestCard as it shows most relevant info. A custom AdminRequestCard could be made. */}
        ...
        <div className="text-center py-12 bg-card rounded-lg shadow">
          ...
          <h2 className="text-xl font-semibold text-foreground mb-2">Nenhum Pedido no Sistema</h2>
          <p className="text-muted-foreground">Ainda não há pedidos registrados por agricultores.</p>
        </div>
      )}
    </PageWrapper>
  );
