...
const fetchAllTechnicianRequests = async (): Promise<AgriRequest[]> => {
  console.log("[PublicAnalyticsPage] Fetching all requests from Firestore for analytics.");
  try {
    const requests = await getAllRequestsSystemWide();
    console.log(`[PublicAnalyticsPage] Fetched ${requests.length} requests from Firestore.`);
    return requests;
  } catch (error) {
    console.error("[PublicAnalyticsPage] Error fetching requests from Firestore for analytics:", error);
    throw error;
  }
};
